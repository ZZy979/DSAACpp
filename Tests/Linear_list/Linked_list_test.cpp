...
TEST_F(Linked_list_test, constructor) {
...
TEST_F(Linked_list_test, size_and_empty) {
    Linked_list<int> l;
    EXPECT_EQ(0, l.size());
    EXPECT_TRUE(l.empty());
    l.insert(0, 8);
...
TEST_F(Linked_list_test, index_of) {
    EXPECT_EQ(4, list.index_of(4));
    EXPECT_EQ(-1, list.index_of(7));
}

TEST_F(Linked_list_test, push_back) {
    Linked_list<int> l;
    EXPECT_EQ(0, l.size());
    l.push_back(8);
    EXPECT_EQ(1, l.size());
...
TEST_F(Linked_list_test, pop_back) {
    Linked_list<int> l;
    l.push_back(8);
    EXPECT_EQ(1, l.size());
    l.pop_back();
...
TEST_F(Linked_list_test, insert) {
...
TEST_F(Linked_list_test, clear) {
    list.clear();
    EXPECT_EQ(0, list.size());
}

TEST_F(Linked_list_test, output) {
    ostringstream oss;
    list.output(oss);
    EXPECT_EQ(std::string("[0, 1, 2, 3, 4, 5]"), oss.str());
}

TEST_F(Linked_list_test, iterator) {
    int v = 0;
    for (Linked_list<int>::iterator p = list.begin(); p != list.end(); ++p)
        EXPECT_EQ(v++, *p);
}

TEST_F(Linked_list_test, set_size) {
    list.set_size(10);
    EXPECT_EQ(6, list.size());
    list.set_size(3);
...
TEST_F(Linked_list_test, subscript) {
    EXPECT_EQ(0, list[0]);
    list[0] = 8;
    EXPECT_EQ(8, list[0]);
}

TEST_F(Linked_list_test, last_index_of) {
...
TEST_F(Linked_list_test, compare) {
...
TEST_F(Linked_list_test, swap) {
    Linked_list<int> a(list), b(list), c;
    c.swap(b);
    EXPECT_TRUE(a == c);
}

TEST_F(Linked_list_test, remove_range) {
